...
void ezUniformBufferPoolVulkan::DeInitialize()
{
  ...
  for (UniformBufferPool* pPool : m_FreePools)
  {
    ...
    pPool->m_Data.Clear();
    ezMemoryAllocatorVulkan::DestroyBuffer(pPool->m_Buffer, pPool->m_Alloc);
    if (pPool->m_StagingBuffer)
    {
      ezMemoryAllocatorVulkan::DestroyBuffer(pPool->m_StagingBuffer, pPool->m_StagingAlloc);
    }
...
void ezUniformBufferPoolVulkan::Reset()
{
  m_Buffer.Clear();
...
void ezUniformBufferPoolVulkan::Submit()
{
...
ezUniformBufferPoolVulkan::UniformBufferPool* ezUniformBufferPoolVulkan::GetFreePool()
{
  ...
  ezUInt32 uiSize = ezMath::Min(1024u * 1024u, uiMaxBufferSize);

  uiSize = ezMemoryUtils::AlignSize(uiSize, (ezUInt32)m_uiAlignment);
  while (uiSize >= uiMaxBufferSize)
  {
    uiSize -= (ezUInt32)m_uiAlignment;
  }
...
void ezUniformBufferPoolVulkan::UniformBufferPool::Submit(ezGALDeviceVulkan* pDevice)
{
  ...
  else
  {
    ...
    VK_ASSERT_DEBUG(ezMemoryAllocatorVulkan::FlushAllocation(m_Alloc, 0, VK_WHOLE_SIZE));
