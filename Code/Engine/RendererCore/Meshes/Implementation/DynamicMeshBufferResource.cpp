...
void ezDynamicMeshBufferResource::UpdateGpuBuffer(ezGALCommandEncoder* pGALCommandEncoder, ezUInt32 uiFirstVertex, ezUInt32 uiNumVertices, ezUInt32 uiFirstIndex, ezUInt32 uiNumIndices, ezGALUpdateMode::Enum mode /*= ezGALUpdateMode::Discard*/)
{
  if (m_bAccessedVB && uiNumVertices > 0)
  {
    ...
    if (mode == ezGALUpdateMode::Discard)
    ...
    pGALCommandEncoder->UpdateBuffer(hVertexBuffer, sizeof(ezDynamicMeshVertex) * uiFirstVertex, m_VertexData.GetArrayPtr().GetSubArray(uiFirstVertex, uiNumVertices).ToByteArray(), ezGALUpdateMode::NoOverwrite);
  }
  ...
  if (m_bAccessedCB && uiNumVertices > 0)
  {
    ...
    pGALCommandEncoder->UpdateBuffer(hColorBuffer, sizeof(ezColorLinearUB) * uiFirstVertex, m_ColorData.GetArrayPtr().GetSubArray(uiFirstVertex, uiNumVertices).ToByteArray(), ezGALUpdateMode::NoOverwrite);
  }
  ...
  if (m_bAccessedIB && uiNumIndices > 0 && m_IndexBuffer.IsInitialized())
  {
    ...
    if (!m_Index16Data.IsEmpty())
    {
      ...
      pGALCommandEncoder->UpdateBuffer(hIndexBuffer, sizeof(ezUInt16) * uiFirstIndex, m_Index16Data.GetArrayPtr().GetSubArray(uiFirstIndex, uiNumIndices).ToByteArray(), ezGALUpdateMode::NoOverwrite);
    }
    else if (!m_Index32Data.IsEmpty())
    {
      ...
      pGALCommandEncoder->UpdateBuffer(hIndexBuffer, sizeof(ezUInt32) * uiFirstIndex, m_Index32Data.GetArrayPtr().GetSubArray(uiFirstIndex, uiNumIndices).ToByteArray(), ezGALUpdateMode::NoOverwrite);
    }
  }
}
